...
extern ssize_t krecv(ksocket_t socket, void *buffer, size_t length, int flags);//接收訊息
...
static long slave_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
{
	...
	switch(ioctl_num){
		case slave_IOCTL_CREATESOCK:// create socket and connect to master
			...
			memset(&addr_srv, 0, sizeof(addr_srv));//初始化值 = 0
			addr_srv.sin_family = AF_INET;
			addr_srv.sin_port = htons(2325);//設定port
...
//call when user is reading from this device
	char msg[BUF_SIZE];
	size_t len;
	len = krecv(sockfd_cli, msg, sizeof(msg), 0);//接收訊息
