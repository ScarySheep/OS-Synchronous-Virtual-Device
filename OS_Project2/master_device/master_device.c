...
#include <asm/highmem.h>
 //debug方便用，virtual memory flag
#ifndef VM_RESERVED
#define VM_RESERVED   (VM_DONTEXPAND | VM_DONTDUMP)
#endif

//自己設定
#define DEFAULT_PORT 2325//port number
#define master_IOCTL_CREATESOCK 0x12345677
#define master_IOCTL_MMAP 0x12345678
#define master_IOCTL_EXIT 0x12345679
#define BUF_SIZE 512
#define MAP_SIZE PAGE_SIZE * 128
typedef struct socket * ksocket_t;//pointer to socket
...
{
//returns the page associated with a virtual address
	vmf->page = virt_to_page(vma->vm_private_data);
	get_page(vmf->page);//increase count of the page by 1
	return 0;
}

//為下方static const struct宣告一個通用的函數使用
...
{
//把IO的Memory投射到process的address space
...
static struct file_operations master_fops = {
	.owner = THIS_MODULE,//在module.h裡，如同this
	.unlocked_ioctl = master_ioctl,//特定的執行選項
	.open = master_open,
	.write = send_msg,
...
static int __init master_init(void)
{
	...
	//設定為可以存取 0~4GB,包括Linux核心所屬的記憶體空間
    old_fs = get_fs();
	set_fs(KERNEL_DS);

	//initialize the master server
	sockfd_srv = sockfd_cli = NULL;
	memset(&addr_cli, 0, sizeof(addr_cli));//初始化值 = 0
	memset(&addr_srv, 0, sizeof(addr_srv));
	addr_srv.sin_family = AF_INET;
	addr_srv.sin_port = htons(DEFAULT_PORT);設定port
	addr_srv.sin_addr.s_addr = INADDR_ANY;//監聽0.0.0.0地址
...
misc_deregister(&master_dev);
	...
	set_fs(old_fs);//執行完系統呼叫後,重新把記憶體空間設定回0~3GB
...
static long master_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
{
	long ret = -EINVAL//Invalid argument
